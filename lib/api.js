...
function normalizeServiceMenuItem(service) {
  if (!service || typeof service !== "object") {
    return null;
  }

  const label =
    firstValue(service, ["name", "title", "service_name", "menu_name", "label"], "") || "";
  const slug = normalizeSlug(firstValue(service, ["slug", "service_slug", "url_slug"], label));
  ...
  return {
    ...
    slug,
    href: `/${slug}`,
    sort_order: Number(service.sort_order ?? service.order ?? service.position ?? 0),
  };
}

export async function getServicesmenu() {
  try {
    const services = await getApiCollection("servicemenu", [], {
      revalidate: 3600,
    });

    return services
      .filter((service) => Number(service?.status) === 1)
      .map(normalizeServiceMenuItem)
      .filter(Boolean)
...
export const getServices = getServicesmenu;

function normalizeCountryItem(country) {
  if (!country || typeof country !== "object") {
    return null;
  }

  const name = firstValue(country, ["name", "country_name", "title", "label"], "");
  ...
  return {
    id: country.id ?? country.country_id ?? name,
    name,
    sort_order: Number(country.sort_order ?? country.order ?? country.position ?? 0),
  };
}

export async function getCountries() {
  try {
    const countries = await getApiCollection("countries", [], {
      revalidate: 3600,
    });

    return countries
      .filter((country) => !Object.prototype.hasOwnProperty.call(country || {}, "status") || Number(country?.status) === 1)
