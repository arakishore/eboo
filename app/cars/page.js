...
function getHomeBanners(banners) {
  const homeBanners = banners.filter(
    (banner) => String(banner?.page || "").trim().toLowerCase() === "car"
  );
...
export default async function CarsPage() {
  ...
  const carGallery = await getServiceGallery("car");
          ...
          <div className="row align-items-stretch">
            <div className="col-lg-6 mb-4 mb-lg-0">
              <div className="service-info-box h-100">
                <h2 className="navy">Where you would like to go</h2>
                <p>
      ...
      <section className="service-gallery-section">
        <div className="container">
          ...
          <ServiceGalleryLightbox
            images={carGallery}
            title="Car"
            emptyMessage="No car gallery images found."
          />
        </div>
      </section>
